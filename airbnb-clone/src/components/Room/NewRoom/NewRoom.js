...
import {faDollarSign,faHome,faPlusCircle,faUser} from '@fortawesome/fontawesome-free-solid';
...
function NewRoom({ roomDispatch,user }) {
    ...
    useEffect(()=>{
        TownService.getAllTowns().then((res)=>{
    ...
    const handleSubmit = e => {
        e.preventDefault();
        console.log("fData",newTownData);
        const { capacity,price,town} = newTownData;
        

        if (validateTownData(newTownData)) {
           setnewTownData({...townData,capacity:Number(capacity),price:Number(price)})
           console.log('data',newTownData);
      
           RoomService.createNewRoom(newTownData).then((res)=>{
            setSuccessful(true);
    ...
    return <div className="signup-form ">
        ...
        <ul>
             ...
             <div hidden={!successful} className="alert alert-success justify-content-center" role="alert">
                Creation réussie !</div>
            ...
            <div className="form-group input-container">
                ...
                <select id='town' onChange={handleChange} className="form-select" aria-label="Choisissez une ville">
                    <option defaultValue>Choisissez une ville</option>
                    {towns.map((town)=><option  key={town.name} value={town.name}>{town.name}</option>)}
            ...
            <div className="form-group input-container">
                ...
                <input type="number" id='capacity' onChange={handleChange} className="form-control input-field" name="capacity" placeholder='0'  />
            </div><br />
            <label>Prix</label>
            ...
            <div className="form-group input-container">
                ...
                <input type="number" id="price" onChange={handleChange} className="form-control input-field" name="price"   />
            </div><br />
          
            <div className="form-group">
                <button type="submit" className="btn btn-primary col-12 btn-lg btn-block">Ajouter   <FontAwesomeIcon key={"plane2"} icon={faPlusCircle} /></button>
